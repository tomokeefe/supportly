...
export default async function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      {/* Industries */}
      <section id="industries" className="bg-taupe">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-20 lg:py-28">
          <div className="max-w-xl mb-16">
            <div className="editorial-rule mb-6" />
            ...
            <p className="text-[--color-text-secondary]">
              Pre-built workflows and knowledge templates for the verticals that
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.values(VERTICALS).map((v) => (
              <Link
                key={v.slug}
                href={`/for/${v.slug}`}
                className="bg-white border border-border rounded-xl p-8 hover:border-dark accent-hover group"
              >
                <h3 className="text-base font-semibold text-dark mb-2 group-hover:text-vermillion accent-hover">
                  {v.industry}
                </h3>
                <p className="text-sm text-[--color-text-secondary] leading-relaxed line-clamp-2">
                  {v.metaDescription}
                </p>
                <span className="inline-block mt-4 text-sm text-vermillion font-medium">
                  Learn more &rarr;
                </span>
              </Link>
            ))}
